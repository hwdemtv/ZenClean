...
for i, line in enumerate(lines):
    if "list[Path] =" in line:
        idx_start = i
    if "Path(_USER_DOWNLOADS)," in line:
        idx_end = i

if idx_start != -1 and idx_end != -1:
    lines = lines[:idx_start + 1] + [new_targets_lines + "\n]\n"] + lines[idx_end + 2:]
    
    # 检查是否加上 _USER_PROFILE
    user_prof_added = False
    for i, line in enumerate(lines):
        if "_USER_PROFILE = os.path.expandvars" in line:
            user_prof_added = True
            break
    if not user_prof_added:
        lines.insert(0, '_USER_PROFILE = os.path.expandvars(r"%USERPROFILE%")\n')

    with open(settings_path, 'w', encoding='utf-8') as f:
        f.writelines(lines)
    print("REPLACED SUCCESSFULLY")
else:
    print(f"NOT FOUND: {idx_start}, {idx_end}")
